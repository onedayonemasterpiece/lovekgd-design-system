...
| `OV-57` | use bounded versioned fixture pools shared by Astro fixture mode and Penpot; do not copy full production listings into design-system pages | `SOT_DEFINED / FESTIVALS_REDUCTION_PENDING` | `NO` | New relevant IdeaHub packet `voice-20260829-072140-eed2c8df` was found at HEAD `eaac25f…` and taken into work. Contract `catalog/reconstruction-atlas/v1/design-system-reference-fixtures-ov57.v1.json` reuses the existing eight-event golden corpus, the exact three factual clubs and the exact-seven Artifact product collection. Festivals must replace the current all-21 owner projection with seven factual festivals in three representative `1/4/2` rows, driven by the same Astro fixture profile. Dense/full listing validation stays in Astro. |
