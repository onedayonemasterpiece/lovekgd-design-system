...
| `OV-54` | cross-cutting Penpot tokens for colors, typography, spacing, radii, elevation and sizing | `PENPOT_FOUNDATIONS + SEVEN COMPONENT MIGRATION BATCHES / GLOBAL MIGRATION IN_PROGRESS` | `NO` | Page `20` projects the source-conformant foundation set; the current set has `97` tokens. Seven bounded migrations now cover Announcements, shared navigation/social-proof/discovery roots, exact-seven Artifact collections, the responsive Interest Club card, Personal Feed owners, desktop Search query roots and all eight mobile Search lifecycle owners. Batch `07` read back `32/32` `radius.20` properties at revision `2815` with unchanged geometry and `validate()=[]`. `space.4` remains a documented Plugin API persistence defect; global coverage and post-binding visual export remain open. |
