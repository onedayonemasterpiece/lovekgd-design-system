...
| `OV-54` | cross-cutting Penpot tokens for colors, typography, spacing, radii, elevation and sizing | `PENPOT_FOUNDATIONS + THIRTEEN COMPONENT MIGRATION BATCHES / GLOBAL MIGRATION IN_PROGRESS` | `NO` | Page `20` projects the source-conformant foundation set; the current set has `97` tokens. Thirteen bounded migrations now cover Announcements, shared navigation/social-proof/discovery roots, exact-seven Artifact collection/card/owner roots, the responsive Interest Club card, Favorites/Personal Feed owners, Search/Event Detail roots and the canonical Home/Date/Weekend owners. Batches `12–13` bind nine owner roots to `radius.20` (`36/36`) through revision `2832`, one settled page at a time; batch `13` adds Popular desktop, Collections mobile and Exhibitions desktop. A new exact census uses the 34 board IDs from executable round-trip bindings: all 34 exist; 24 have a nonzero uniform radius, 15 are bound and 9 remain; 10 zero-radius roots are explicit non-applicable cases. Search now counts the factual entered-query results owners rather than the obsolete idle-anonymous representatives. Latest census revision is `2832`, geometry is unchanged and `validate()=[]`. `space.4`, the remaining 9 owner roots and post-binding visual export remain open. |
