...
| `OV-54` | cross-cutting Penpot tokens for colors, typography, spacing, radii, elevation and sizing | `PENPOT_FOUNDATIONS + TWELVE COMPONENT MIGRATION BATCHES / GLOBAL MIGRATION IN_PROGRESS` | `NO` | Page `20` projects the source-conformant foundation set; the current set has `97` tokens. Twelve bounded migrations now cover Announcements, shared navigation/social-proof/discovery roots, exact-seven Artifact collection/card/owner roots, the responsive Interest Club card, Favorites/Personal Feed owners, Search/Event Detail roots and the canonical Home/Date/Weekend owners. Batch `12` binds six owner roots to `radius.20` (`24/24`) through revision `2825`, one settled page at a time. A new exact census uses the 34 board IDs from executable round-trip bindings: all 34 exist; 24 have a nonzero uniform radius, 12 are bound and 12 remain; 10 zero-radius roots are explicit non-applicable cases. Search now counts the factual entered-query results owners rather than the obsolete idle-anonymous representatives. Latest census revision is `2826`, geometry is unchanged and `validate()=[]`. `space.4`, the remaining 12 owner roots and post-binding visual export remain open. |
