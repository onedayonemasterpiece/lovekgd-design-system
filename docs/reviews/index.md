...
| `OV-57` | use bounded versioned fixture pools shared by Astro fixture mode and Penpot; do not copy full production listings into design-system pages | `ASTRO_AND_PENPOT_STRUCTURALLY_VERIFIED / VISUAL_EXPORT_BLOCKED` | `NO` | New relevant IdeaHub packet `voice-20260829-072140-eed2c8df` was found at HEAD `eaac25f…` and taken into work. Contract `catalog/reconstruction-atlas/v1/design-system-reference-fixtures-ov57.v1.json` reuses the existing eight-event golden corpus, the exact three factual clubs and the exact-seven Artifact product collection. Astro commit `dec0d11b3` exposes the explicit shared profile and its preview build verified seven cards in `1/4/2` rows. Penpot revision `2866` now uses native bounded desktop/mobile owners with the same seven cards; the old all-21 raster owners are hidden and marked superseded. Structural read-back and root token bindings pass. Final raster comparison remains honestly open because the Penpot exporter is still in the active 504 incident and was not blindly retried. Dense/full listing validation stays in Astro. |
