...
| `OV-02` | `40.3A`: cover crop без полей, letterbox и растяжения | `MISMATCH_CONFIRMED / LOWEST_OWNER_DECISION_PENDING` | `NO` | Current Astro does not blind-cover protected media: OCR/document/unsafe assets use a content-sized authored-ratio `contain` wrapper. Targeted readback confirms `event.real.6936` and `event.real.6652` still use wrong `93×112` Penpot cover crops instead of the factual `75×112` contain geometry. A lowest-owner authored-contain media state must be certified before linked swaps; page-local resizing is forbidden. Contract: `catalog/reconstruction-atlas/v1/mobile-rail-media-astro-penpot-reconciliation.v1.json` |
...
| `OV-06` | `63.15 Artifacts`: 7/7 artifacts и none/subset/all expanded, hover/focus, selected-detail states | `PARTIAL_AS_IS_PROJECTION / OWNER_TARGET_BLOCKED` | `NO` | Page `63.15` now has three bounded native boards with 13 linked instances: all 8 factual Amber rail states, production unavailable / nonprod 0-of-5 / found 1-of-5 collection states, and desktop/mobile selected-detail dialogs. Exact readback, idempotency `created=0` and `validate()=[]` pass; focused export returned Penpot HTTP 504. The owner-required seven identities and none/subset/all-expanded behavior remain blocked because current Astro defines 1 implemented Amber artifact in 5 slots, seven reference images represent six concepts, and the separate Focus Lab defines 12 eggs; none may be relabelled or invented. Receipt: `evidence/recovery-20260828/penpot/artifacts-astro-as-is-owner-projection-receipt.v1.json` |
