...
| `OV-57` | use bounded versioned fixture pools shared by Astro fixture mode and Penpot; do not copy full production listings into design-system pages | `ASTRO_PROFILE_VERIFIED / PENPOT_FESTIVALS_REDUCTION_PENDING` | `NO` | New relevant IdeaHub packet `voice-20260829-072140-eed2c8df` was found at HEAD `eaac25f…` and taken into work. Contract `catalog/reconstruction-atlas/v1/design-system-reference-fixtures-ov57.v1.json` reuses the existing eight-event golden corpus, the exact three factual clubs and the exact-seven Artifact product collection. Astro commit `dec0d11b3` now exposes the explicit shared profile and its preview build verified seven cards in `1/4/2` rows; receipt `evidence/recovery-20260829/astro/ov57-festival-fixture-profile.v1.json`. Penpot must still replace the current all-21 projection with that bounded composition. Dense/full listing validation stays in Astro. |
