...
Continuation through Penpot revision `2816` expands that bounded migration to
eight recorded batches without changing source geometry. The active set now
...
lifecycle owner roots. The Event Detail batch adds its mobile owner plus the
portrait-hero, portrait-viewer, transport and continuation roots. The two
latest batches persisted `52/52` `radius.20` properties after HTTP `504`
unknown outcomes and delayed readback; no blind retry was performed and
`validate()=[]`. Global migration and a stable post-binding visual export
remain open, so `processed` stays `NO`.
