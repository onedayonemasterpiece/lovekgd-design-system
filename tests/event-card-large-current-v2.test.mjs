...
for(const entry of reg.cases){
 ...
 const row=json(c);assert.equal(row.lifecycle_status,'active_blocked');assert.equal(row.penpot_binding.binding_status,'bound');assert(!JSON.stringify(row).includes('pending-'));assert.equal(row.current_penpot_structural_readback.status,'verified_found');assert.equal(row.current_penpot_structural_readback.file_revision,1408);assert(existsSync(join(current,row.current_penpot_structural_readback.facts_path)));if(row.viewport.id==='desktop-1280'){assert.equal(row.viewport.container_width,380);assert.notEqual(row.viewport.container_width,474);assert.equal(row.penpot_binding.revision,1316);assert.equal(row.penpot_binding.shape_id,row.current_penpot_structural_readback.shape_id);}
}
