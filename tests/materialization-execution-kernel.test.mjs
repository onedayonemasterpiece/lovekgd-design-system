...
function createFixture(page = false) {
  ...
  const index = { schema: 'kenigevents.resolved-render-case-index.v1', control_generation: 4, generator: 'test', input_manifest: 'test', astro_ref: 'f'.repeat(40), geometry_proof: { path: 'geometry.json', content_sha256: '1'.repeat(64), file_sha256: '2'.repeat(64) }, cases: bindings, content_sha256: null };
...
function createAdapter() { const roots = new Map(); const writes = []; return { roots, writes, lookup: async (key) => roots.get(key), write: async (operation) => { writes.push(operation); const value = { output_uuid: `output-${operation.sequence}` }; roots.set(operation.idempotence_key, value); return value; }, rollback: async () => ({ rolled_back: true }) }; }
