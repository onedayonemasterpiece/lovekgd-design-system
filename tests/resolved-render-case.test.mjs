...
const indexPath = 'catalog/ui-conformance/free-collection/g3/resolved/resolved-cases.index.json';
const indexFileSha256 = sha256(fs.readFileSync(path.join(root, indexPath)));
const planPath = path.join(root, 'catalog/ui-conformance/free-collection/g3/evidence-plan.json');
...
function validatePlan(document) {
  ...
  assert.equal(document.control_generation, 3);
...
test('resolved-case file or binding mismatch fails closed', () => {
  ...
  const copiedRoot = path.join(temp, 'catalog/ui-conformance/free-collection/g3/resolved');
  fs.mkdirSync(path.dirname(copiedRoot), { recursive: true });
  fs.cpSync(path.join(root, 'catalog/ui-conformance/free-collection/g3/resolved'), copiedRoot, { recursive: true });
  ...
  assert.throws(() => loadResolvedCaseIndex(temp, 'catalog/ui-conformance/free-collection/g3/resolved/resolved-cases.index.json', sha256(fs.readFileSync(copiedIndex)), [index.cases[0].case_id]), /RESOLVED_CASE_FILE_HASH_MISMATCH/u);
});
