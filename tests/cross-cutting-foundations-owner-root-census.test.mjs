...
test('OV-54 reports current progress rather than calling partial owner-root coverage complete', () => {
  assert.equal(census.coverage.bound_uniform_nonzero_radius_roots, 15);
  assert.equal(census.coverage.unbound_uniform_nonzero_radius_roots, 9);
  ...
  assert.equal(contract.penpot_projection.canonical_owner_root_census.bound, 15);
  assert.equal(contract.penpot_projection.canonical_owner_root_census.remaining, 9);
