...
test('OV-54 reports current progress rather than calling partial owner-root coverage complete', () => {
  assert.equal(census.coverage.bound_uniform_nonzero_radius_roots, 11);
  assert.equal(census.coverage.unbound_uniform_nonzero_radius_roots, 13);
  ...
  assert.equal(contract.penpot_projection.canonical_owner_root_census.bound, 11);
  assert.equal(contract.penpot_projection.canonical_owner_root_census.remaining, 13);
