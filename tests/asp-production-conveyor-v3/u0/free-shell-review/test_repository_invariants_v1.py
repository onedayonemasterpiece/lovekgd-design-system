...
PACKAGE = json.loads((ROOT / "catalog/asp-production-conveyor-v3/u0/free-shell-review" / "U-FREE-SHELL-REVIEW-PAGE-R1.package.v1.json").read_text(encoding="utf-8"))
...
class PackageInvariantTests(unittest.TestCase):
    ...
    def test_executor_is_concrete_and_guarded(self):
        runtime = (ROOT / "scripts/asp-production-conveyor-v3/u0/free-shell-review" / "native_runtime_v1.js").read_text(encoding="utf-8")
        entry = (ROOT / "scripts/asp-production-conveyor-v3/u0/free-shell-review" / "native_executor_v1.js").read_text(encoding="utf-8")
    ...
    def test_atlas_extension_has_no_page_order(self):
        request = (ROOT / "catalog/asp-production-conveyor-v3/u0/free-shell-review" / "ASP_ATLAS_EXTENSION_REQUEST_V1.md").read_text(encoding="utf-8")
    ...
    def test_lane_specific_acceptance(self):
        self.assertEqual(len(PACKAGE["page_units"]), 1)
        unit = PACKAGE["page_units"][0]
        self.assertEqual(len(unit["components"]), 7)
        self.assertEqual(len(unit["specimens"]), 6)
        nav = next(item for item in unit["components"] if item["component_id"] == "U-SHELL-MOBILE-BOTTOM-NAVIGATION")
        self.assertEqual([item["label"] for item in nav["anatomy"]], ["Афиша", "Даты", "Поиск", "Для меня"])
        self.assertFalse(PACKAGE["acceptance"]["sticky_runtime_simulation"])
        self.assertFalse(PACKAGE["acceptance"]["new_shell_design"])
