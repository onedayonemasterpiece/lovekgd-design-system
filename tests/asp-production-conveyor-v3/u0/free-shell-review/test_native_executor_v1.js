...
const { MemoryPenpotAdapter, MemoryStorage, validatePackage } = require('../../../../scripts/asp-production-conveyor-v3/u0/free-shell-review/native_runtime_v1');
const { packageDefinition, setupPackage } = require('../../../../scripts/asp-production-conveyor-v3/u0/free-shell-review/setup_v1');
const { run } = require('../../../../scripts/asp-production-conveyor-v3/u0/free-shell-review/native_executor_v1');
...
test('Free shell contract carries seven exact component boundaries and six review scenarios', () => {
  assert.equal(packageDefinition.page_units.length, 1);
  const unit = packageDefinition.page_units[0];
  assert.equal(unit.components.length, 7);
  assert.equal(unit.specimens.length, 6);
  assert.deepEqual(
    unit.components.find((item) => item.component_id === 'U-SHELL-MOBILE-BOTTOM-NAVIGATION').anatomy.map((item) => item.label),
    ['Афиша', 'Даты', 'Поиск', 'Для меня'],
  );
  assert.equal(packageDefinition.acceptance.sticky_runtime_simulation, false);
  assert.equal(packageDefinition.acceptance.new_shell_design, false);
});
