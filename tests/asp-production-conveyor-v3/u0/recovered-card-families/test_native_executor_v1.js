...
const { MemoryPenpotAdapter, MemoryStorage, validatePackage } = require('../../../../scripts/asp-production-conveyor-v3/u0/recovered-card-families/native_runtime_v1');
const { packageDefinition, setupPackage } = require('../../../../scripts/asp-production-conveyor-v3/u0/recovered-card-families/setup_v1');
const { run } = require('../../../../scripts/asp-production-conveyor-v3/u0/recovered-card-families/native_executor_v1');
...
test('recovered package contains exactly five non-EventCard-clone family pages', () => {
  assert.deepEqual(packageDefinition.families, [
    'U-CARD-COMPACT',
    'U-CARD-FESTIVAL',
    'U-CARD-CLUB',
    'U-CARD-ARTIFACT',
  ...
  assert.equal(packageDefinition.page_units.length, 5);
  assert.ok(packageDefinition.page_units.every((unit) => unit.components.length === 1));
  assert.ok(packageDefinition.page_units.every((unit) => unit.components[0].explicit_difference_from_eventcard));
  assert.equal(packageDefinition.acceptance.eventcard_clones, 0);
  assert.equal(packageDefinition.acceptance.old_penpot_uuid_lineage, 0);
});
