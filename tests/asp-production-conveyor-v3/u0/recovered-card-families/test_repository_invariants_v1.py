...
PACKAGE = json.loads((ROOT / "catalog/asp-production-conveyor-v3/u0/recovered-card-families" / "U-RECOVERED-CARD-FAMILIES.package.v1.json").read_text(encoding="utf-8"))

class PackageInvariantTests(unittest.TestCase):
    def test_identity_and_authority(self):
        self.assertEqual(PACKAGE["package_id"], "U-RECOVERED-CARD-FAMILIES")
    ...
    def test_executor_is_concrete_and_guarded(self):
        runtime = (ROOT / "scripts/asp-production-conveyor-v3/u0/recovered-card-families" / "native_runtime_v1.js").read_text(encoding="utf-8")
        entry = (ROOT / "scripts/asp-production-conveyor-v3/u0/recovered-card-families" / "native_executor_v1.js").read_text(encoding="utf-8")
    ...
    def test_atlas_extension_has_no_page_order(self):
        request = (ROOT / "catalog/asp-production-conveyor-v3/u0/recovered-card-families" / "ASP_ATLAS_EXTENSION_REQUEST_V1.md").read_text(encoding="utf-8")
    ...
    def test_lane_specific_acceptance(self):
        self.assertEqual(PACKAGE["families"], [
            "U-CARD-COMPACT",
            "U-CARD-FESTIVAL",
            "U-CARD-CLUB",
            "U-CARD-ARTIFACT",
        ...
        self.assertEqual(len(PACKAGE["page_units"]), 5)
        for unit in PACKAGE["page_units"]:
            self.assertEqual(len(unit["components"]), 1)
            self.assertTrue(unit["components"][0]["explicit_difference_from_eventcard"])
        self.assertEqual(PACKAGE["acceptance"]["eventcard_clones"], 0)
        self.assertEqual(PACKAGE["acceptance"]["old_penpot_uuid_lineage"], 0)
