...
class T(unittest.TestCase):
 def test_exact_binding_and_atlas_geometry(self):
  ...
  source=b.decode();self.assertIn('function f0Utf8',source);self.assertIn('async function f0Sha256',source);self.assertNotIn('TextEncoder',source);self.assertNotIn('crypto.subtle',source);self.assertNotIn('globalThis.crypto',source);self.assertEqual(d['runtime_compatibility'],{'hash_implementation':'pure-js-f0Sha256','utf8_implementation':'pure-js-f0Utf8','requires_TextEncoder':False,'requires_crypto':False,'requires_crypto_subtle':False,'geometry_epsilon':0.001,'epsilon_scoped_to':['x','y','width','height','containment','non_overlap']});self.assertLessEqual(d['runtime_compatibility']['geometry_epsilon'],0.01);self.assertIn('GEOM_EPSILON=0.001',source)
