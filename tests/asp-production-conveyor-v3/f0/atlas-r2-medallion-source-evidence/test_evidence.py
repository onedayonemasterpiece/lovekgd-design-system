...
def main() -> int:
    ...
    assert measurements["root"]["height"] == 920
    ...
    assert measurements["template"]["occupied_rows"] == 2
    assert measurements["template"]["content_height"] == 600
    assert measurements["template"]["final_row_tracks"] == [3, 4]
    ...
    assert [cell["cell_bounds"][0] for cell in measurements["cells"][6:]] == [976, 1256]
    assert all(cell["cell_bounds"][1] == 568 for cell in measurements["cells"][6:])

    assert 'width="2176" height="920"' in svg
    ...
    assert validation["render"]["svg"]["height"] == 920
    assert validation["render"]["png"]["width"] == 2176
    assert validation["render"]["png"]["height"] == 920
