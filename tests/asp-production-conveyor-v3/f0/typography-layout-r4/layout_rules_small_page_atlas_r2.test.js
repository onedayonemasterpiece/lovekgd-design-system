...
} = require('../../../../scripts/asp-production-conveyor-v3/f0/typography-layout-r4/layout_rules_small_page_atlas_r2.js');
const { allNodes, createNativeLikePenpot } = require('./native_like_penpot_double.js');
...
test('package executor loads in a plugin-like browser global without CommonJS', () => {
  ...
  vm.runInNewContext(
    readFileSync('scripts/asp-production-conveyor-v3/f0/typography-layout-r4/typography_atlas_r2_native_runtime.js', 'utf8'),
    context,
  );
  vm.runInNewContext(
    readFileSync('scripts/asp-production-conveyor-v3/f0/typography-layout-r4/layout_rules_small_page_atlas_r2.js', 'utf8'),
    context,
  );
  assert.equal(
