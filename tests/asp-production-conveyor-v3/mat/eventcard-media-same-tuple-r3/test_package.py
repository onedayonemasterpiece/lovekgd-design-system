...
PROVIDER = ROOT / "scripts/asp-production-conveyor-v3/mat/eventcard-media-same-tuple-r3/provider-envelope.v1.json"
...
class PackageTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.provider = json.loads(PROVIDER.read_text(encoding="utf-8"))
    ...
    def test_standalone_bundle_identity_assets_and_forbidden_runtime_tokens(self):
        ...
        self.assertEqual(meta["global"], "KenigEventsD0EventcardMediaR3StandaloneV7")
        ...
        self.assertIn(self.provider["packageHead"], self.bundle)
        self.assertIn(self.provider["bundleSha256"], self.bundle)
        self.assertIn(self.provider["bundleBlobSha1"], self.bundle)
        self.assertIn(self.provider["operationIdentitySha256"], self.bundle)
