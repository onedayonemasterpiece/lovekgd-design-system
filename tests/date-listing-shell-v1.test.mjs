...
for (const id of ['DL-032', 'DL-033', 'DL-034', 'DL-035']) assert(decisions.decisions.some((item) => item.id === id));
for (const id of ['shell.mobile-menu.close-tag', 'action.shell.mobile-menu.login', 'action.shell.mobile-menu.favorites', 'action.shell.mobile-menu.share', 'shell.mobile-menu.utility', 'action.shell.mobile-menu.row']) assert(components.shell_components.some((item) => item.id === id));
