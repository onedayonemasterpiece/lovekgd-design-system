...
def main():
    ...
    if set(comments)!={11,12,13} or any(x['resolved'] for x in comments.values()): fail('EMF_COMMENT_STATE_INVALID','/owner_comment_constraints','threads 11, 12 and 13 must remain recorded and unresolved until Penpot and Git match')
    ...
    print(json.dumps({'status':'valid','contract':str(cp.relative_to(root)),'contract_payload_sha256':actual,'slots':len(ids),'comments_open':[11,12,13]},indent=2))
