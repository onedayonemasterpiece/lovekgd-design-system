...
def main():
    ...
    historical={key:comments.get(key) for key in (11,12,13)}
    if any(value is None or not value['resolved'] for value in historical.values()): fail('EMF_COMMENT_STATE_INVALID','/owner_comment_constraints','historical threads 11, 12 and 13 must remain recorded with explicit resolved dispositions; later review threads may remain unresolved until their visual fix exists')
