...
const runtimeFamily=fonts.font_files.find((font)=>font.family==='DejaVu Sans');assert(runtimeFamily,'RUNTIME_FONT_FAMILY_MISSING:DejaVu Sans');
const runtimeFaces=[...new Set(fonts.regions.flatMap((region)=>region.platform_fonts).filter((font)=>font.familyName==='DejaVu Sans').map((font)=>font.postScriptName))];
for(const face of ['DejaVuSans','DejaVuSans-Bold'])assert(runtimeFaces.includes(face),`RUNTIME_FONT_FACE_MISSING:${face}`);
const resolvedFont=(style)=>({family:'DejaVu Sans',face:Number.parseInt(style.fontWeight,10)>=700?'DejaVuSans-Bold':'DejaVuSans',source_manifest_sha256:artifacts.find((a)=>a.path.endsWith('/runtime-font-manifest.json')).sha256});
