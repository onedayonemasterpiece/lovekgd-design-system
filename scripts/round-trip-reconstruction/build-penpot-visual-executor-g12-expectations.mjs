...
const fixtures=Object.fromEntries(fixtureDoc.fixtures.map((f)=>[f.fixture_id,{title:f.preview_event.title,occurrence:null,city:f.preview_event.city,venue:f.preview_event.venue_name,place:[f.preview_event.city,f.preview_event.venue_name].filter(Boolean).join(' · '),event_type:f.preview_event.event_type,status:f.preview_event.status_label,likes:f.preview_event.likes_count||0,shares:f.preview_event.shares_count||0,media:{...fixtureDoc.media.find((m)=>m.fixture_id===f.fixture_id),width:f.preview_event.image_assets?.[0]?.width||null,height:f.preview_event.image_assets?.[0]?.height||null}}]));
const caseRegions=regionDoc.regions.filter((r)=>r.level==='L2_EVENTCARD');
const cases=caseRegions.map((region)=>{const fixtureId=region.fixture_ids[0];const card=cardModel(region,fixtureId);const occurrence=card.slots.occurrence.text;fixtures[fixtureId].occurrence=occurrence;return{case_id:region.id,variant:region.id.replace(/^eventcard\./,'').replace(/\.8006$|\.2182$/,''),fixture_id:fixtureId,viewport:region.id.includes('.desktop')?'desktop':'mobile',source_region_id:region.id,source_capture_sha256:captures.captures.find((c)=>c.region_id===region.id).sha256,...card};});
const groups=regionDoc.regions.filter((r)=>r.level==='L3_ROWS_AND_GROUPS').map((region)=>{
  ...
  const instances=region.fixture_ids.map((fixtureId)=>{const model=cardModel(region,fixtureId);const id=fixtureId.replace('event.real.','');const card=all.find((n)=>n.data['data-event-card']!==undefined&&n.data['data-event-id']===id);return{...model,fixture_id:fixtureId,box:relBox(card.box,region.root.box)};});
...
const expectations={schema:'kenigevents.penpot-executor-independent-current-a-expectations.g12.v1',status:'CURRENT_A_USED_VALUES_AUTHORITY',provenance:{astro_binding_sha256:null,regions_sha256:artifacts.find((a)=>a.path.endsWith('/regions.json')).sha256,fixture_input_sha256:artifacts.find((a)=>a.path.endsWith('/fixture-input.json')).sha256,font_manifest_sha256:artifacts.find((a)=>a.path.endsWith('/runtime-font-manifest.json')).sha256,captures_manifest_sha256:artifacts.find((a)=>a.path.endsWith('/captures-manifest.json')).sha256},component_model:{semantic_identity:'component.event-card.free-collection',master_count:1,variant_container:true,variant_axis:'structural_context',variants:cases.map((c)=>c.variant)},assets,fixtures,cases,groups,runtime_fonts:fonts};
