...
function installFreeSeptemberV2(penpot, penpotUtils, storage) {
  ...
  function repairOwner(viewport,state='full') {
    assertContext();const component=componentByIdentity(OWNER_PATH,ownerName(viewport,state))||componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));if(!component)throw new Error(`missing owner ${viewport}/${state}`);const root=component.mainInstance(),desk=viewport==='desktop',h=state==='full'?(desk?3338.34375:4270.4375):(desk?1200:844);place(root,root.x,root.y,desk?1280:390,h);const body=byName(root,/^linked Free collection body/),header=byName(root,/^linked Shell header/);place(body,desk?50:12,state==='scrolled'?(desk?-560:-499):57,desk?1180:366,desk?2599.484375:4213.4375);place(header,0,0,desk?1280:390,57);
