/** Native, resumable Penpot projection for the September v3 closure scenarios.
 *
 * Exact payload authority:
 *   catalog/fixtures/ui-reference-events/v2/
 * Exact Astro consumer scenario:
 *   free-collection-september-desktop-v3 / mobile-v3
 *
...
const SCENARIO_IDS = { desktop:'free-collection-september-desktop-v3', mobile:'free-collection-september-mobile-v3' };
const LEGACY_SCENARIO_ID = 'free-collection-september-desktop-v2';
const REFERENCE_ISO = '2026-08-29T14:00:00+02:00';
const REFERENCE_DATE = '2026-08-29';
...
const PACKED_PLACE = {
  2182:'Калининград · Калининградская…',
  6711:'Светлогорск · Информационно-…',
};
const GEOMETRY = {
  desktop: {
...
function installFreeSeptemberV2(penpot, penpotUtils, storage) {
  const scenarioFor = (viewport) => SCENARIO_IDS[viewport];
  const bodyName = (viewport) => `viewport=${viewport};fixture=${REFERENCE_DATE};scenario=${scenarioFor(viewport)}`;
  const legacyBodyName = (viewport) => `viewport=${viewport};fixture=2026-09-01;scenario=${LEGACY_SCENARIO_ID}`;
  const ownerName = (viewport,state) => `viewport=${viewport};state=${state};scenario=${scenarioFor(viewport)}`;
  const legacyOwnerName = (viewport,state) => `viewport=${viewport};state=${state};scenario=${LEGACY_SCENARIO_ID}`;
  const assertContext = () => {
  ...
  async function ensureBody(viewport) {
    assertContext(); const name=bodyName(viewport);let existing=componentByIdentity(BODY_PATH,name);if(!existing){existing=componentByIdentity(BODY_PATH,legacyBodyName(viewport));if(existing)existing.name=name;}if(existing){const main=existing.mainInstance();main.setPluginData('scenario-id',scenarioFor(viewport));main.setPluginData('reference-iso',REFERENCE_ISO);return {created:false,id:existing.id,main:main.id};}
    ...
    try {
      const isDesk=viewport==='desktop',w=isDesk?1180:366,h=isDesk?2599.484375:4213.4375;const root=board(null,`${BODY_PATH} / ${name}`,isDesk?14800:16200,3000,w,h,'#f8f1e7');root.clipContent=false;root.setPluginData('scenario-id',scenarioFor(viewport));root.setPluginData('reference-iso',REFERENCE_ISO);makeHero(root,viewport,badge);
      if(isDesk){
  ...
  async function ensureOwner(viewport,state='full') {
    assertContext(); const name=ownerName(viewport,state);let existing=componentByIdentity(OWNER_PATH,name);if(!existing){existing=componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));if(existing)existing.name=name;}if(existing){const main=existing.mainInstance();main.setPluginData('scenario-id',scenarioFor(viewport));main.setPluginData('reference-iso',REFERENCE_ISO);return {created:false,id:existing.id,main:main.id};}const body=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport)),header=componentById(viewport==='desktop'?DESKTOP_HEADER_ID:MOBILE_HEADER_ID),footer=componentById(DESKTOP_FOOTER_ID),nav=componentById(MOBILE_NAV_ID);if(!body||!header)throw new Error('owner prerequisites missing');const block=penpot.history.undoBlockBegin();
    try {
      const desk=viewport==='desktop',w=desk?1280:390,h=state==='full'?(desk?3338.34375:4270.4375):(desk?1200:844),x=state==='full'?(desk?13000:14350):(desk?14880:16240),y=0;const root=board(null,`${OWNER_PATH} / ${name}`,x,y,w,h,'#f8f1e7');root.clipContent=state!=='full';root.setPluginData('scenario-id',scenarioFor(viewport));root.setPluginData('reference-iso',REFERENCE_ISO);root.setPluginData('review-status','NOT_REVIEWED');
      const b=body.instance();b.name=`linked Free collection body / ${viewport}`;root.appendChild(b);place(b,desk?50:12,state==='scrolled'?(desk?-643:-463):57,desk?1180:366,desk?2599.484375:4213.4375);
  ...
  function repairBody(viewport) {
    assertContext();const component=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport));if(!component)throw new Error(`missing ${viewport} body`);const root=component.mainInstance();const desk=viewport==='desktop';place(root,root.x,root.y,desk?1180:366,desk?2599.484375:4213.4375);
    ...
    }else{place(hero,0,101.390625,366,436.25);place(byName(hero,/Hero \/ green atmosphere/),246,0,120,436.25);const exact=[[/Eyebrow/,19,27,226,20],[/Title/,19,54.96875,226,64],[/Lead/,19,134.125,226,122],[/Criteria/,19,267.84375,226,66],[/Updated/,19,345.625,226,66],[/Medallion/,251,71,96,96]];for(const [q,x,y,w,h] of exact)place(byName(hero,q),x,y,w,h);setText(hero,/Lead/,'Все актуальные события с подтверждённым бесплатным входом, включая продолжающиеся выставки.');setText(hero,/Criteria/,'Как собрана: Событие активно, ещё не закончилось, а в выгрузке афиши вход подтверждён как бесплатный.');setText(hero,/Updated/,'Данные афиши обновлены 2026-08-29; подборка рассчитана на 2026-08-29. Это не личный сохранённый поиск.');place(events,0,569.640625,366,1421.59375);place(byName(events,/Results heading/),13,21,340,26);for(const [i,id] of [8006,8200].entries())place(byName(events,new RegExp(`event.real.${id}`)),13,66.90625+i*673.640625,340,660.046875);place(exhibitions,0,2011.234375,366,2038.203125);place(byName(exhibitions,/Eyebrow/),13,21,340,20);place(byName(exhibitions,/Results heading/),13,46.5625,340,26);place(byName(exhibitions,/Group note/),13,82.078125,340,77);setText(exhibitions,/Group note/,'Продолжающиеся выставки отделены от событий с конкретным временем, чтобы их было проще просматривать.');for(const [i,id] of [2182,6711,7609].entries())place(byName(exhibitions,new RegExp(`event.real.${id}`)),13,178.859375+i*617.3125,340,603.71875);}
    return {viewport,componentId:component.id,mainId:root.id};
  }

  function repairOwner(viewport,state='full') {
    assertContext();const component=componentByIdentity(OWNER_PATH,ownerName(viewport,state))||componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));if(!component)throw new Error(`missing owner ${viewport}/${state}`);const root=component.mainInstance(),desk=viewport==='desktop',h=state==='full'?(desk?3338.34375:4270.4375):(desk?1200:844);place(root,root.x,root.y,desk?1280:390,h);const body=byName(root,/^linked Free collection body/),header=byName(root,/^linked Shell header/);place(body,desk?50:12,state==='scrolled'?(desk?-643:-463):57,desk?1180:366,desk?2599.484375:4213.4375);place(header,0,0,desk?1280:390,57);
  ...
  function ensureBreadcrumb(viewport) {
    const name=`viewport=${viewport};scenario=${scenarioFor(viewport)}`;
  ...
  function repairHeroExact(viewport) {
    const component=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport));if(!component)throw new Error(`missing ${viewport} body`);
    ...
    updated.characters='Данные афиши обновлены 2026-08-29; подборка рассчитана на 2026-08-29. Это не личный сохранённый поиск.';
  ...
  function hydrateUnifiedSlot(slot,id) {
    ...
    if(storage.unifiedGoldenEventCards?.applyFixture){storage.unifiedGoldenEventCards.applyFixture(card,id);return card;}
    setText(card,/^Content \/ Event title$/,spec.title);setText(card,/^Label \/ instance content$/,spec.type);setText(card,/^(?:Content \/ Event occurrence|schedule)$/,spec.occurrence);setText(card,/^Value \/ instance content$/,spec.price);setText(card,/^Content \/ Event place$/,spec.group==='exhibitions'?(PACKED_PLACE[id]||spec.place):spec.place);
  ...
  function bindBodyCardToUnified(id,viewport) {
    ...
    const spec=EVENTS[id],bodyComponent=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport));if(!bodyComponent)throw new Error(`missing body ${viewport}`);
  ...
  function repairOwnersExact() {
    const receipts=[];
    for(const viewport of ['desktop','mobile'])for(const state of ['top','scrolled','full']){
      const component=componentByIdentity(OWNER_PATH,ownerName(viewport,state))||componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));if(!component)throw new Error(`missing owner ${viewport}/${state}`);const root=component.mainInstance(),desktop=viewport==='desktop',h=state==='full'?(desktop?3338.34375:4270.4375):(desktop?1200:844);
      place(root,root.x,root.y,desktop?1280:390,h);root.clipContent=true;root.fills=[{fillColorGradient:{type:'linear',startX:0,startY:0,endX:0,endY:1,width:1,stops:[{color:'#fff8ed',offset:0},{color:'#fbf7ef',offset:.42},{color:'#f7efe3',offset:1}]},fillOpacity:1}];
      const body=byName(root,/^linked Free collection body/),header=byName(root,/^linked Shell header/);place(body,desktop?50:12,state==='scrolled'?(desktop?-643:-463):57,desktop?1180:366,desktop?2599.484375:4213.4375);place(header,0,0,desktop?1280:390,57);
  ...
  function migrateScenarioClock() {
    assertContext();
    const migrated={bodies:[],owners:[],breadcrumbs:[]};
    for(const viewport of ['desktop','mobile']){
      let body=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport));
      if(!body)throw new Error(`missing body for clock migration ${viewport}`);
      body.name=bodyName(viewport);const bodyRoot=body.mainInstance();bodyRoot.setPluginData('scenario-id',scenarioFor(viewport));bodyRoot.setPluginData('reference-iso',REFERENCE_ISO);
      migrated.bodies.push({viewport,componentId:body.id,mainId:bodyRoot.id,name:body.name});
      const legacyBreadcrumb=`viewport=${viewport};scenario=${LEGACY_SCENARIO_ID}`;
      const currentBreadcrumb=`viewport=${viewport};scenario=${scenarioFor(viewport)}`;
      const breadcrumb=componentByIdentity(BREADCRUMB_PATH,currentBreadcrumb)||componentByIdentity(BREADCRUMB_PATH,legacyBreadcrumb);
      if(breadcrumb){breadcrumb.name=currentBreadcrumb;migrated.breadcrumbs.push({viewport,componentId:breadcrumb.id,name:breadcrumb.name});}
      for(const state of ['top','scrolled','full']){
        const owner=componentByIdentity(OWNER_PATH,ownerName(viewport,state))||componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));
        if(!owner){if(state==='top')continue;throw new Error(`missing owner for clock migration ${viewport}/${state}`);}
        owner.name=ownerName(viewport,state);const root=owner.mainInstance();root.setPluginData('scenario-id',scenarioFor(viewport));root.setPluginData('reference-iso',REFERENCE_ISO);
        migrated.owners.push({viewport,state,componentId:owner.id,mainId:root.id,name:owner.name});
      }
    }
    return migrated;
  }
  ...
  async function repairExactParity() {
    assertContext();const block=penpot.history.undoBlockBegin();try{const scenarioClock=migrateScenarioClock();const hero=['desktop','mobile'].map(repairHeroExact);const sections=['desktop','mobile'].map(repairSectionsExact);const cards=bindBodyCardsToUnified();const footerShare=repairFooterShareExact();const owners=repairOwnersExact();const reviewCanvas=organizeReviewCanvas();return {scenarioClock,hero,sections,cards,footerShare,owners,reviewCanvas,validation:await penpot.currentFile.validate()};}finally{penpot.history.undoBlockFinish(block);}
  }
  ...
  async function readback() {
    assertContext();const cards=['desktop','mobile'].flatMap((viewport)=>ORDER.map((id)=>{const body=componentByIdentity(BODY_PATH,bodyName(viewport))||componentByIdentity(BODY_PATH,legacyBodyName(viewport)),slot=body?byName(body.mainInstance(),new RegExp(`^linked EventCard / event\\.real\\.${id} /`)):null;return {id,viewport,componentId:slot?.component?.()?.id||null,slotId:slot?.id||null,fixture:slot?.getPluginData?.('fixture-id')||null};}));const owners=['desktop','mobile'].flatMap((viewport)=>['top','scrolled','full'].map((state)=>{const c=componentByIdentity(OWNER_PATH,ownerName(viewport,state))||componentByIdentity(OWNER_PATH,legacyOwnerName(viewport,state));return {viewport,state,componentId:c?.id||null,mainId:c?.mainInstance()?.id||null};}));return {scenarios:SCENARIO_IDS,referenceIso:REFERENCE_ISO,order:ORDER,cards,owners,validation:await penpot.currentFile.validate()};
  }

  storage.freeSeptemberV2={ensureBody,ensureOwner,repairBody,repairOwner,repairAll,repairHeroExact,repairSectionsExact,bindBodyCardToUnified,bindBodyCardsToUnified,repairFooterShareExact,repairOwnersExact,organizeReviewCanvas,migrateScenarioClock,repairExactParity,readback,constants:{FILE_ID,PAGE_ID,BODY_PATH,OWNER_PATH,UNIFIED_CARD_PATH,UNIFIED_CARD_NAMES,SCENARIO_IDS,REFERENCE_ISO,REFERENCE_DATE,ORDER,REVIEW_IDS}};
  return {installed:true,methods:Object.keys(storage.freeSeptemberV2),scenarios:SCENARIO_IDS,referenceIso:REFERENCE_ISO,order:ORDER};
}
