...
contract.status='MOBILE_DESKTOP_LOADING_RESULTS_MATERIALIZED_LIFECYCLE_STATES_OPEN';
...
contract.coverage={
  ...
  open_required_states:['mobile-validation','mobile-empty','mobile-error-retry','mobile-stale','mobile-load-more'],
};
contract.visual_evidence.result='MOBILE_VISUAL_PASS_DESKTOP_STRUCTURAL_VERIFIED_EXPORT_DEFERRED';
...
receipt.structural_readback.round_trip_state_owners=states.map(s=>({viewport:s.viewport,state:s.state,component_id:s.component_id,main_id:s.main_id,direct_children:s.children.map(c=>c.shape_id)}));
receipt.visual_qa.result='MOBILE_PASS_DESKTOP_STRUCTURAL_VERIFIED_LIFECYCLE_OPEN';
...
for(const region of archetype.regions){
  if(region.region_id==='search.query-field') region.source_exact_instances=states.map(s=>({viewport:s.viewport,state:s.state,shape_id:s.children[1].shape_id,component:s.children[1].component}));
  ...
  if(region.region_id==='search.collection-links') region.source_exact_instances=states.map(s=>({viewport:s.viewport,state:s.state,shape_id:s.children[2].shape_id,component:s.children[2].component}));
}
bindings.correction_overlays??=[];
const overlay={archetype_id:archetype.archetype_id,astro_commit:ASTRO,contract:archetype.source_exact_correction,penpot_page_id:PAGE,penpot_revision:REVISION,review_items:['OV-47','OV-48'],scope:'mobile-desktop-loading-results'};
...
console.log(`${BINDINGS}: registered ${states.length} Search source-exact state owners; lifecycle states remain open`);
