...
function finalizeCurrent({casePath,runDir,actualTuplePath,runId}){
 ...
 for(const name of ['astro.png','penpot.png','overlay-50.png','diff.png'])if(review.reviewed_files?.[name]!==fileDigest(join(dir,name))&&review.reviewed_pixel_sha256?.[name]!==pixelDigest(join(dir,name)))errors.push(`agent review image-content mismatch: ${name}`);
