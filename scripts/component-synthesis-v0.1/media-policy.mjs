...
export function validateEventMediaPolicy({ root, applicationRefs, fixtureIds, entityIds, hierarchyEdges, materializableIds }) {
  ...
  const atlas = proof.crop_atlas;
  demand(atlas.variant_count === 19 && atlas.logical_non_lab_profile_count === 15 && atlas.variant_axis.join('|') === 'profile', 'ACS_MEDIA_PENPOT_ATLAS', 'media-policy', PATHS.mediaPenpotProof, '$.crop_atlas', 'visual crop atlas must expose 19 native projections over 15 non-lab profiles');
  demand(unique(atlas.variants.map((row) => row.profile_ref)) && unique(atlas.variants.map((row) => row.stable_id)) && atlas.variants.every((row) => row.source_preview && row.crop_window_or_no_crop_overlay && row.result_preview && row.canonical === false && row.accepted === false && row.promotion_ready === false), 'ACS_MEDIA_PENPOT_ATLAS', 'media-policy', PATHS.mediaPenpotProof, '$.crop_atlas.variants', 'crop atlas variants must be unique, comparable and candidate-only');
  demand(atlas.variants.some((row) => row.fit_projection === 'contain') && atlas.variants.some((row) => row.fit_projection === 'no-site-crop') && atlas.variants.some((row) => row.fit_projection.includes('conflict-open')), 'ACS_MEDIA_PENPOT_ATLAS', 'media-policy', PATHS.mediaPenpotProof, '$.crop_atlas.variants', 'atlas must visibly distinguish contain, no-crop and conflict-open projections');

  return { resolver, rules, profiles, cases, proof, counts: { rules: rules.length, profiles: profiles.length, applications: assignedApplications.length, cases: cases.length, penpot_variants: proof.variant_cases.length, penpot_linked_rail_instances: proof.rail_proof.nested_instances.length, penpot_crop_atlas_variants: atlas.variant_count } };
}
