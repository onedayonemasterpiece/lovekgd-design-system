...
ROW_GAP = 24
COLS = 6
OCCUPIED_ROWS = (len(MEMBERSHIP) + COLS - 1) // COLS
CONTENT_HEIGHT = OCCUPIED_ROWS * CELL_H + max(0, OCCUPIED_ROWS - 1) * ROW_GAP
ROOT_HEIGHT = GRID_Y + CONTENT_HEIGHT + 64
ROOT = (0, 0, 2176, ROOT_HEIGHT)
CANDIDATE_ROOT = (32, 32, 2112, ROOT_HEIGHT - 64)
MASTER_COLUMN = (64, GRID_Y, 320, CONTENT_HEIGHT)
ASSET_SNAPSHOT_DIR = Path("reports/asp-production-conveyor-v3/atlas-v2/source-bound/f0-medallions/source-assets")
...
def load_inputs(repo: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    snapshots = {item["path"]: item for item in json.loads((repo / ASSET_SNAPSHOT_DIR / "manifest.v1.json").read_text())["assets"]}
    ...
    for index, (stable_id, binding_id) in enumerate(MEMBERSHIP, start=1):
        ...
        source_file = repo / ASSET_SNAPSHOT_DIR / Path(source_path).name
        ...
        source_blob = git_blob_bytes(raw)
        assert source_blob == snapshot["git_blob_sha1"]
        assert len(raw) == snapshot["bytes"] and sha256(raw) == snapshot["sha256"]
...
def build_svg(records: list[dict[str, Any]]) -> tuple[bytes, list[dict[str, Any]]]:
    out: list[str] = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="2176" height="{ROOT_HEIGHT}" viewBox="0 0 2176 {ROOT_HEIGHT}" data-root-width="2176" data-root-height="{ROOT_HEIGHT}" data-template="FOUNDATION_ASSET_GRID_DENSE_V2" data-template-columns="6" data-version="F0-SOURCE-BOUND-V2-BALANCED">',
    ...
    for i, record in enumerate(records, start=1):
        ...
        # The master ledger is compacted to the same two occupied grid rows;
        # it cannot force a phantom third documentation row or taller root.
        y = 374 + (i - 1) * 62
    ...
    global_order = 0
    final_row_count = len(records) % COLS
    final_row_start = len(records) - final_row_count if final_row_count else len(records)
    final_row_offset = (COLS - final_row_count) // 2 if final_row_count else 0
    for i, record in enumerate(records):
        row = i // COLS
        col = final_row_offset + (i - final_row_start) if i >= final_row_start else i % COLS
...
def render_png(svg_bytes: bytes) -> bytes:
    ...
    raw = cairosvg.svg2png(bytestring=svg_bytes, output_width=2176, output_height=ROOT_HEIGHT)
...
def build_outputs(repo: Path) -> dict[str, bytes]:
    records, context = load_inputs(repo)
    ...
    measurements = {
        ...
        "root": {"bounds": list(ROOT), "width": 2176, "height": ROOT_HEIGHT},
    ...
    validation = {
        ...
        "render": {
            "svg": {"width": 2176, "height": ROOT_HEIGHT, "bytes": len(svg_bytes), "sha256": sha256(svg_bytes)},
            "png": {"width": 2176, "height": ROOT_HEIGHT, "bytes": len(png_a), "sha256": sha256(png_a)},
        },
        "penpot_reads": 0,
        "penpot_mutations": 0,
