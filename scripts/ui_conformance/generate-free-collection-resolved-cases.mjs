...
const INPUTS = 'catalog/ui-conformance/free-collection/g3/resolved-case-inputs.json';
const CASES = 'catalog/ui-conformance/free-collection/g3/cases';
const OUTPUT = 'catalog/ui-conformance/free-collection/g3/resolved';
...
export function validateResolvedCase(document) {
  ...
  assert(document.control_generation === 3, 'RESOLVED_CASE_GENERATION_MISMATCH');
...
export async function generate({ root, astroRoot, check = false }) {
  ...
  assert(inputs.control_generation === 3, 'INPUT_GENERATION_MISMATCH');
  ...
  for (const filename of caseFiles) {
    ...
    assert(spec.schema === 'kenigevents.ui-conformance-case.v1' && spec.control_generation === 3, `CASE_SCHEMA_MISMATCH:${spec.case_id}`);
