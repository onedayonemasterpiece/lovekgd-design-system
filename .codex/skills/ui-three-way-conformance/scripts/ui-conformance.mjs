...
import {
  ...
  preflightTuple, readJson, refreshRunManifest, sha256, stableJson, stageAstroDiagnostic, validateAgentReview, validateCase, validateExceptionRegistry,
...
try {
  ...
  if (command === 'help' || values.help) process.stdout.write(help);
    ...
    const board = existsSync(join(runDir, 'astro.png')) && existsSync(join(runDir, 'penpot.png'))
      ? createComparisonBoard({ runDir, caseRow, finalStatus: final.status, geometryBlockers: blockers, pixelRatio: metrics?.difference_ratio ?? null, fontStatus: (preflight.blockers || []).includes('BLOCKED_FONT_ENV') ? 'BLOCKED' : 'OK', tupleStatus: (preflight.blockers || [preflight.status]).join(' + '), runId })
    ...
    if (errors.length) fail(errors); else { writeJson(join(runDir, 'telegram-readback-receipt.json'), receipt); refreshRunManifest(runDir, { status: 'published-read-back', publishedMessageId: receipt.message_id, retentionClass: 'published-24h' }); print({ ok: true, message_id: receipt.message_id }); }
