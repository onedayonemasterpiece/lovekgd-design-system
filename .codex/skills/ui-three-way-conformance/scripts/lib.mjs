...
export function validateCase(row) {
  ...
  const pendingPenpot = p.binding_status === 'pending-materialization';
  expect(['bound', 'pending-materialization'].includes(p.binding_status), 'invalid Penpot binding_status', errors);
  ...
  expect(pendingPenpot ? p.board_or_component_id === null : UUID.test(p.board_or_component_id || ''), 'invalid Penpot board_or_component_id', errors);
  expect(pendingPenpot ? p.board_label === null || /^Board /u.test(p.board_label || '') : /^Board /u.test(p.board_label || ''), 'board_label must explicitly start with Board', errors);
  expect(pendingPenpot ? p.revision === null || Number.isInteger(p.revision) || (typeof p.revision === 'string' && p.revision.length > 0)
    : Number.isInteger(p.revision) || (typeof p.revision === 'string' && p.revision.length > 0), 'Penpot revision is required when bound', errors);
  expect(nullableSha(p.export_sha256), 'invalid Penpot export_sha256', errors);
  expect(pendingPenpot ? p.export_sha256 === null : SHA256.test(p.export_sha256 || ''), 'Penpot export_sha256 is required when bound', errors);
...
export function preflightTuple(caseRow, actual) {
  ...
  for (const row of actual.verified_assets || []) {
    if (!SHA256.test(row.expected_sha256 || '') || row.actual_sha256 !== row.expected_sha256 || !Number.isInteger(row.byte_length) || row.byte_length <= 0) {
      if (!blockers.includes('BLOCKED_ASSET_MISMATCH')) blockers.push('BLOCKED_ASSET_MISMATCH');
      remainingReasons.push(`Verified asset bytes mismatch: ${row.asset_id || 'unknown'}`);
    }
  }
...
export function createComparisonBoard({ runDir, caseRow, finalStatus, geometryBlockers = 0, pixelRatio = null, fontStatus = 'OK', tupleStatus = 'READY', runId }) {
  ...
  const header = 194; const footer = 122; const label = 42; const bodyHeight = Math.max(ad.height, pd.height);
  ...
  const eventLine = `Event: ${caseRow.event_title || 'untitled fixture'}`;
  const foot = `Geometry blockers: ${geometryBlockers} · Pixel diff: ${pixelRatio === null ? 'n/a' : pixelRatio.toFixed(6)} · Fonts: ${fontStatus}`;
  const tupleLine = `Tuple: ${tupleStatus}`; const runLine = `Run ${runId} · ${caseRow.authority_mode} · ${caseRow.contract_sha256.slice(0, 10)} · Owner: AWAITING_REVIEW`;
  ...
  imageConvert([
    ...
    '-pointsize', '15', '-annotate', `+${pad}+148`, eventLine,
...
export function createBlockedComparisonBoard({ runDir, caseRow, reason, runId }) {
  ...
  const header = 194; const label = 42; const footer = 104; const bodyHeight = Math.max(ad.height, 260); const totalHeight = header + label + bodyHeight + footer + pad;
  ...
  const commandArgs = [
    ...
    '-pointsize', '15', '-annotate', `+${pad}+148`, `Event: ${caseRow.event_title || 'untitled fixture'}`,
    '-font', 'DejaVu-Sans-Bold', '-pointsize', '18', '-annotate', `+${pad}+${header + 29}`, 'ASTRO',
  ...
  commandArgs.push(
    ...
    '-pointsize', '10', '-annotate', `+${pad}+${header + label + bodyHeight + 60}`, `Run ${runId} · ${caseRow.authority_mode} · ${caseRow.contract_sha256.slice(0, 10)} · Owner: AWAITING_REVIEW`,
    '-strip', output,
  );
...
export function publicationRetentionClass(finalStatusValue) {
