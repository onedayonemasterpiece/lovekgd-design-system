...
function loadResolvedCaseIndex(root, indexPath, expectedFileSha256, requiredCaseIds = []) {
  ...
  if (index.schema !== INDEX_SCHEMA || index.control_generation !== 4) fail('RESOLVED_INDEX_SCHEMA_MISMATCH', indexPath);
  ...
  for (const caseId of requiredCaseIds) {
    ...
    if (document.schema !== CASE_SCHEMA || document.case_id !== caseId || document.control_generation !== 4) fail('RESOLVED_CASE_SCHEMA_MISMATCH', caseId);
