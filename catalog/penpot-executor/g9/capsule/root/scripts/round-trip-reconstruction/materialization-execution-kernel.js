...
const PHASE_B_AUTHORIZATION = 'ACCEPTED_FOR_PHASE_B_PRODUCTION';
...
function loadCanonicalInputs(options) {
  ...
  assertPreflight(['PROMOTABLE', 'READY_FOR_W0_PROMOTION'].includes(bundle.promotion_state), 'BUNDLE_NOT_PROMOTABLE', 'bundle state is invalid');
  assertPreflight(bundle.control_generation !== UNRESOLVED, 'BUNDLE_CONTROL_MISMATCH', 'bundle generation unresolved');
  ...
  delete identityTarget.runtime_native_target;
  assertPreflight(!UUID_RE.test(JSON.stringify(identityTarget)), 'TARGET_UUID_FORBIDDEN', 'target UUID identity');
  if (target.runtime_native_target !== undefined) {
    assertPreflight(target.runtime_native_target?.run_specific === true, 'TARGET_UUID_FORBIDDEN', 'native target must be run-specific');
    assertPreflight(UUID_RE.test(target.runtime_native_target.file_id || '') && UUID_RE.test(target.runtime_native_target.page_id || ''), 'NATIVE_TARGET_ID_MISSING', 'file/page');
  }
  ...
  const accepted = control.accepted_bundles?.[bundle.bundle_id];
  const directAuthorization = bundle.control_generation === control.generation && accepted === bundle.revision;
  const phaseBAuthorization = Boolean(
    bundle.promotion_state === 'READY_FOR_W0_PROMOTION'
    && control.generation > bundle.control_generation
    && accepted?.revision === bundle.revision
    && accepted?.source_control_generation === bundle.control_generation
    && accepted?.source_promotion_state === bundle.promotion_state
    && accepted?.bundle_sha256 === bundleFile.sha256
    && accepted?.authorization === PHASE_B_AUTHORIZATION
  );
  assertPreflight(directAuthorization || phaseBAuthorization, 'BUNDLE_NOT_ACCEPTED', 'exact bundle authorization');
  ...
  return {
    ...
    resolvedCases,
    acceptedBundleAuthorization: { direct: directAuthorization, phase_b: phaseBAuthorization, record: accepted },
  };
}
...
function buildPlan(loaded) {
  ...
  for (const scope of loaded.bundle.materialization_scopes) {
    operations.push({
      ...
      case_id: String(scope.source_ref).replace(/^resolved-case:/u, ''),
...
class MemoryLeaseStore {
  ...
  async validate(key, identity) {
    return { active: this.active.get(key) === identity, holder: this.active.get(key) || null };
  }
}
...
function createReceipt(options, now) {
  return {
    ...
    capsule_manifest_sha256: options.capsuleManifestSha256 || null,
    adapter: null,
    ...
    case_roots: [],
    mutations: { completed: [], rolled_back: [], unreverted: [] },
    cleanup_scope: [],
    idempotence_keys: [],
...
async function executeMaterialization(rawOptions) {
  ...
  try {
    ...
    assertPreflight(options.mode !== 'production' || loaded.bundle.promotion_state === 'PROMOTABLE' || loaded.acceptedBundleAuthorization.phase_b, 'BUNDLE_NOT_PROMOTABLE', 'candidate lacks exact Phase-B authorization');
    ...
    Object.assign(receipt, {
      ...
      bundles: [{ id: loaded.bundle.bundle_id, revision: loaded.bundle.revision, source_promotion_state: loaded.bundle.promotion_state, authorization: loaded.acceptedBundleAuthorization.record, dependencies: loaded.bundle.dependencies || [] }],
  ...
  if (!adapter?.lookup || !adapter?.write || !adapter?.rollback || !leaseStore?.acquire || !leaseStore?.validate || !leaseStore?.release || !cancellationProvider?.read) {
    receipt.preflight = { passed: false, code: 'RUNTIME_BINDING_MISSING', message: 'adapter/lease/cancel' };
    return finishReceipt(receipt, 'FAILED_PREFLIGHT', now);
  }
  receipt.adapter = adapter.identity || null;
  receipt.runtime_native_target = loaded.target.runtime_native_target || null;
  ...
  const freshCheck = async (operation, mutationIndex, rollback = false) => {
    const cancellation = await cancellationProvider.read(loaded.control.cancellation.binding_id);
    const leaseState = await leaseStore.validate(leaseKey, leaseIdentity);
    const check = {
      ...
      lease_active: leaseState?.active === true,
      lease_identity: leaseIdentity,
      rollback,
    };
    receipt.pre_write_checks.push(check);
    receipt.cancellation_checks.push(check);
    if (check.generation !== loaded.control.generation || (!rollback && check.cancelled) || !check.lease_active) {
      const error = new Error('fresh generation/cancellation/lease check failed');
      error.code = check.cancelled ? 'CANCELLED' : (check.generation !== loaded.control.generation ? 'GENERATION_DRIFT' : 'LEASE_LOST');
      throw error;
    }
    return check;
  };
  try {
    for (let index = 0; index < plan.operations.length; index += 1) {
      ...
      if (existing) {
        ...
        if (operation.case_id) receipt.case_roots.push({ case_id: operation.case_id, semantic_id: operation.semantic_id, output_uuid: existing.output_uuid || null, resolved_case_sha256: loaded.resolvedCases.cases[operation.case_id].content_sha256 });
        continue;
      }
      const runtimeOperation = {
        ...operation,
        resolved_case: loaded.resolvedCases.cases[operation.case_id] || null,
        component_contract: loaded.sources.find((source) => source.role === 'component_contract')?.document || null,
        runtime_native_target: loaded.target.runtime_native_target || null,
      };
      if (!adapter.perMutationChecks) await freshCheck(operation, 1);
      const recordMutation = (mutation) => {
        const item = { operation_sequence: operation.sequence, semantic_id: operation.semantic_id, ...mutation };
        completedMutations.push(item);
        receipt.mutations.completed.push(item);
      };
      const result = await adapter.write(runtimeOperation, {
        beforeMutation: (mutationIndex) => freshCheck(operation, mutationIndex),
      ...
      if (!result?.mutations_reported_live) for (const mutation of result?.mutations || []) recordMutation(mutation);
      ...
      if (operation.case_id) receipt.case_roots.push({ case_id: operation.case_id, semantic_id: operation.semantic_id, output_uuid: recorded.output_uuid, resolved_case_sha256: loaded.resolvedCases.cases[operation.case_id].content_sha256 });
    }
  } catch (error) {
    terminalState = error.code === 'CANCELLED' ? 'CANCELLED' : 'FAILED_WRITE';
    receipt.failure = { code: error.code || 'WRITE_ERROR', message: error.message };
    for (const mutation of [...completedMutations].reverse()) {
      try {
        const result = await adapter.rollback(mutation, {
          beforeMutation: () => freshCheck({ sequence: mutation.operation_sequence }, `rollback:${mutation.mutation_id || mutation.native_id}`, true),
        });
        if (result?.rolled_back) receipt.mutations.rolled_back.push({ ...mutation, rollback_id: result.rollback_id || null });
        else receipt.mutations.unreverted.push({ ...mutation, reason: result?.reason || 'ROLLBACK_UNAVAILABLE' });
      } catch (rollbackError) {
        receipt.mutations.unreverted.push({ ...mutation, reason: rollbackError.code || rollbackError.message });
      }
    }
    if (receipt.mutations.unreverted.length) terminalState = 'FAILED_PARTIAL_STATE';
    else if (completedMutations.length && terminalState !== 'CANCELLED') terminalState = 'FAILED_ROLLED_BACK';
  } finally {
