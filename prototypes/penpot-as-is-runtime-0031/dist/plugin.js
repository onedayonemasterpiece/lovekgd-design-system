(() => {
  ...
  const UI_SHA = '00e00cac159ca61fdd4de3dea368460386ef723d';
  const REPOSITORY = 'onedayonemasterpiece/lovekgd-design-system';
  const UI_URL = `https://raw.githack.com/${REPOSITORY}/${UI_SHA}/prototypes/penpot-as-is-runtime-0031/dist/ui.html`;
  ...
  penpot.ui.open('LoveKGD Runtime Review · 003.1', UI_URL, { width: 500, height: 820 });
  ...
  const progress = (value) => send({ type: 'sync-progress', ...value });
  ...
  function normalize(value, depth = 0, seen = new WeakSet()) {
    ...
    for (const key of keys) {
      if (key === 'stack' && typeof value[key] === 'string') {
        result[key] = value[key].split('\n').slice(0, 12).join('\n');
        continue;
      }
      try {
        const item = value[key];
        if (item !== undefined) result[key] = normalize(item, depth + 1, seen);
      } catch (error) {
  ...
  async function populate(board, element, context) {
    const image = await uploadMedia(element, context);
  ...
  async function apply(catalogInput, loadMetrics = {}) {
    ...
    if (!inspected.changeCount) {
      send({
        type: 'sync-report',
        result: { pages: 0, created: 0, replaced: 0, reviewed: 0, moved: 0, removed: 0 },
        metrics: { loadMs: loadMetrics.totalMs || 0, inspectMs: inspected.inspectMs, stageMs: 0, commitMs: 0 },
      });
      return;
    }
    ...
    try {
      ...
      for (const value of staged.values()) {
        try { value.board.remove(); } catch {}
      }
      if (originalPage) await penpot.openPage(originalPage);
      if (!error.__diagnosticSent) {
        const value = reportDiagnostic({ phase: 'staging', catalog }, error);
        error.__incidentId = value.incidentId;
      }
      const wrapped = new Error(`staging_failed:${error.__incidentId || 'unknown'}`);
    ...
    try {
      for (let index = 0; index < inspected.items.length; index += 1) {
      ...
      for (let index = 0; index < stageItems.length; index += 1) {
        ...
        progress({ phase: 'commit', index: index + 1, total: stageItems.length, elementId: element.id, pageId: element.pageId });
      ...
      progress({ phase: 'verify', index: catalog.elements.length, total: catalog.elements.length });
      ...
      for (const value of snapshots.reverse()) {
        try {
          await penpot.openPage(value.page);
          restore(value);
        } catch {}
      }
      for (const value of staged.values()) {
      ...
      if (originalPage) {
        try { await penpot.openPage(originalPage); } catch {}
      }
      const value = reportDiagnostic({ phase: 'commit-verify', catalog }, error);
  ...
  penpot.ui.onMessage(async (message) => {
    try {
      ...
      if (!error.__diagnosticSent) reportDiagnostic({ phase: `message:${message?.type || 'unknown'}`, catalog: message?.catalog }, error);
