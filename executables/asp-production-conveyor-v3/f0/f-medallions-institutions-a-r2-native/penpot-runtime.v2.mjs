...
const REQUIRED_PROVENANCE = ['session_id','task_id','writer_id','package_id','package_head','package_tree','triggered_by','lease_token','lease_expires_at'];
...
async function readActive(context,authorization){
  ...
  for(const key of REQUIRED_PROVENANCE)invariant(String(marker[key])===String(authorization.provenance[key]),'ACTIVE_LEASE_IDENTITY_DRIFT',{key,expected:authorization.provenance[key],actual:marker[key]});
  invariant(marker.package_id===authorization.package_id&&marker.package_id===SPEC.package_id,'ACTIVE_LEASE_PACKAGE_DRIFT',{expected:authorization.package_id,actual:marker.package_id});
  const expiresAt=Date.parse(marker.lease_expires_at),now=typeof context.now==='function'?Number(await context.now()):Date.now();
  invariant(Number.isFinite(expiresAt)&&Number.isFinite(now)&&expiresAt>now,'ACTIVE_LEASE_EXPIRED',{lease_expires_at:marker.lease_expires_at,now});
  return marker;
}
function validateAuthorization(context,authorization,projection){
  invariant(authorization?.schema_version==='kenigevents.d0-bounded-native-authorization.v1','AUTH_SCHEMA_MISMATCH');
  invariant(authorization.package_id===SPEC.package_id&&authorization.package_head===context.exactPackageHead&&authorization.package_tree===context.exactPackageTree,'AUTH_PACKAGE_MISMATCH');
  invariant(/^[0-9a-f]{40}$/u.test(context.exactPackageHead)&&/^[0-9a-f]{40}$/u.test(context.exactPackageTree),'EXACT_PROVIDER_TUPLE_REQUIRED');
  ...
  invariant(authorization.provenance.package_id===authorization.package_id,'AUTH_PROVENANCE_PACKAGE_MISMATCH');
  invariant(authorization.provenance.package_head===authorization.package_head,'AUTH_PROVENANCE_HEAD_MISMATCH');
  invariant(authorization.provenance.package_tree===authorization.package_tree,'AUTH_PROVENANCE_TREE_MISMATCH');
  invariant(authorization.provenance.triggered_by===authorization.triggered_by,'AUTH_TRIGGER_DRIFT');
}
...
function exactAssetHandle(handle,asset){
  ...
  const payload=expected.media_type==='image/svg+xml'?utf8(handle.svg):handle.data;invariant(payload.byteLength===expected.bytes,'ASSET_PAYLOAD_BYTES_DRIFT',expected.path);invariant(sha256Portable(payload)===expected.sha256,'ASSET_PAYLOAD_SHA256_DRIFT',expected.path);
  return handle;
}
...
function createComponent(penpot,shape){return penpot.library.local.createComponent([shape]);}
async function nativeCreate(context,authorization,counter,label,operation){await readActive(context,authorization);const value=await operation();counter.count+=1;counter.labels.push(label);return value;}
async function instanceOf(context,authorization,component,counter){const shape=await nativeCreate(context,authorization,counter,'component.instance',()=>component.instance());invariant(shape,'LINKED_INSTANCE_CREATE_FAILED');return shape;}
...
async function createOperation(context,authorization,operation,index,counter){
  ...
  if(operation.kind==='master'){
    const rail=parentShape(page,`${LOGICAL_PAGE_ID}.master-rail`),main=setGeometry(await nativeCreate(context,authorization,counter,'penpot.createBoard',()=>penpot.createBoard()),{x:72,y:264+assetIndex*104,width:96,height:96,name:`Medallion/${asset.short_name}`});main.clipContent=true;mark(main,masterId,'asset-master-main',{'semantic-id':asset.semantic_id,'binding-id':asset.binding_id,'source-sha256':asset.source.sha256});append(rail,main);const component=await nativeCreate(context,authorization,counter,'library.local.createComponent',()=>createComponent(penpot,main));component.name=`Medallion/${asset.short_name}`;component.path='KenigEvents / Medallions / Institutions A';mark(component,masterId,'asset-master',{'semantic-id':asset.semantic_id,'binding-id':asset.binding_id,'source-sha256':asset.source.sha256});return;
  }
  ...
  if(operation.kind==='shape'){
    const shape=await nativeCreate(context,authorization,counter,'penpot.createEllipse',()=>penpot.createEllipse());setGeometry(shape,{x:main.x,y:main.y,width:96,height:96,name:`${asset.short_name} · background`});shape.fills=[{fillColor:asset.presentation.background}];shape.strokes=[{strokeColor:asset.presentation.ring,strokeWidth:2}];mark(shape,operation.id,operation.role,{'source-bound':'true'});append(main,shape);return;
  }
  if(operation.kind==='artwork'){
    const handle=await exactAsset(context,asset);let shape;
    let image=null;
    if(asset.source.media_type==='image/svg+xml'){shape=await nativeCreate(context,authorization,counter,'penpot.createShapeFromSvg',()=>penpot.createShapeFromSvg(handle.svg));invariant(shape,'SVG_IMPORT_RETURNED_NULL',asset.source.path);}
    else{image=await nativeCreate(context,authorization,counter,'penpot.uploadMediaData',()=>penpot.uploadMediaData(`${asset.semantic_id}-${asset.source.sha256.slice(0,12)}`,handle.data,asset.source.media_type));invariant(image&&typeof image.data==='function','IMAGE_UPLOAD_RETURNED_INVALID_DATA',asset.source.path);const nativeBytes=await image.data();invariant(nativeBytes instanceof Uint8Array&&nativeBytes.byteLength===asset.source.bytes,'NATIVE_IMAGE_DATA_BYTES_DRIFT',asset.source.path);invariant(sha256Portable(nativeBytes)===asset.source.sha256,'NATIVE_IMAGE_DATA_SHA256_DRIFT',asset.source.path);shape=await nativeCreate(context,authorization,counter,'penpot.createRectangle',()=>penpot.createRectangle());shape.fills=[{fillOpacity:1,fillImage:image}];}
    setGeometry(shape,{x:main.x+8,y:main.y+8,width:80,height:80,name:`${asset.short_name} · exact source artwork`});mark(shape,operation.id,operation.role,{'source-path':asset.source.path,'source-ref':asset.source.ref,'source-git-blob':asset.source.git_blob_sha1,'source-sha256':asset.source.sha256,'source-bytes':String(asset.source.bytes),'source-media-type':asset.source.media_type});append(main,shape);if(image){const leaf=await shape.export({type:'png',scale:1});invariant(leaf instanceof Uint8Array&&leaf.byteLength>32,'NATIVE_IMAGE_LEAF_EXPORT_EMPTY',asset.source.path);sharedSet(shape,'native-image-data-sha256',asset.source.sha256);sharedSet(shape,'native-image-data-bytes',String(asset.source.bytes));sharedSet(shape,'leaf-export-sha256',sha256Portable(leaf));sharedSet(shape,'leaf-export-bytes',String(leaf.byteLength));}return;
  }
  ...
  const shape=await instanceOf(context,authorization,component,counter);configureLinked(shape,component,operation,416+col*(cellWidth+24)+(cellWidth-operation.tier)/2,256+row*(cellHeight+24)+(cellHeight-operation.tier)/2,operation.tier,operation.tier,`${asset.semantic_id}/tier-${operation.tier}`);mark(shape,operation.id,operation.role,{'semantic-slot':'linked_review_instances','semantic-id':asset.semantic_id,'tier-px':String(operation.tier),'linked-order':String(order+1),'source-sha256':asset.source.sha256});append(parentShape(page,`${LOGICAL_PAGE_ID}.dense-grid`),shape);
}
...
export async function projectMedallionsInstitutionsAR2Native(context){
  ...
  invariant(/^[0-9a-f]{40}$/u.test(context.exactPackageHead||'')&&/^[0-9a-f]{40}$/u.test(context.exactPackageTree||''),'EXACT_PROVIDER_TUPLE_REQUIRED');
  const state={schema_version:'kenigevents.f0-medallions-institutions-a-native-projection.v2',package_id:SPEC.package_id,package_head:context.exactPackageHead,package_tree:context.exactPackageTree,source_parent_head:'03188ef9ad579ae24ffba87b6a15e65a567a4869',atlas:{logical_page_id:LOGICAL_PAGE_ID,physical_page_name:PHYSICAL_PAGE_NAME,template_id:SPEC.page.template_id,semantic_slots:['page_header','package_owned_masters','linked_review_instances']},revision:revisionOf(penpot),page_id:page?.id||null,current_page_id:penpot.currentPage?.id||null,cursor,total_operations:PLAN.length,complete:cursor===PLAN.length,inventory:inventory(penpot,page),protected:protectedProjection,mutation_in_flight:false,penpot_mutations:0};
...
export async function executeMedallionsInstitutionsAR2NativePhase(context,authorization){
  const before=await projectMedallionsInstitutionsAR2Native(context);validateAuthorization(context,authorization,before);await readActive(context,authorization);
  const counter={count:0,labels:[]};let completedOperations=0,lastIndex=before.cursor-1;
  try{
    for(let index=before.cursor;index<PLAN.length;index+=1){const cost=phaseCost(PLAN[index]);if(counter.count+cost>MAX_CREATES_PER_PHASE)break;const countBefore=counter.count;await createOperation(context,authorization,PLAN[index],index,counter);invariant(counter.count-countBefore===cost,'NATIVE_CREATE_COST_DRIFT',{index,id:PLAN[index].id,expected:cost,actual:counter.count-countBefore});completedOperations+=1;lastIndex=index;const page=findPage(context.penpot);invariant(operationExists(context.penpot,page,PLAN[index]),'CREATE_NOT_READ_BACK',{index,id:PLAN[index].id});}
  }catch(error){let readback=null,readbackError=null;try{readback=await projectMedallionsInstitutionsAR2Native(context);}catch(projectError){readbackError={code:projectError.code||'PROJECTION_FAILED',detail:projectError.detail||null};}error.unknownOutcome=counter.count>0||Boolean(readback&&(readback.cursor!==before.cursor||readback.revision!==before.revision))||Boolean(readbackError);error.retryAllowed=false;error.requiredNextOperation='DISTINCT_READ_ONLY_PROJECTION';error.readback=readback;error.readbackError=readbackError;error.nativeCreates=counter.count;error.nativeCreateLabels=counter.labels;throw error;}
  const after=await projectMedallionsInstitutionsAR2Native(context);invariant(counter.count<=MAX_CREATES_PER_PHASE,'NATIVE_CREATE_LIMIT_EXCEEDED',counter);invariant(after.cursor===before.cursor+completedOperations,'RESUME_CURSOR_DRIFT',{before:before.cursor,completedOperations,after:after.cursor,lastIndex});invariant(canonical(before.protected)===canonical(after.protected),'PROTECTED_PROJECTION_CHANGED');invariant(after.inventory.detached===0&&after.inventory.screenshot_shapes===0,'NATIVE_NEGATIVE_GATE_FAILED',after.inventory);
  ...
  return{schema_version:'kenigevents.f0-medallions-institutions-a-native-phase-result.v2',package_id:SPEC.package_id,package_head:authorization.package_head,package_tree:authorization.package_tree,state:after.complete?(counter.count===0?'REPLAY_ZERO_CREATED':'COMPLETE_PENDING_DISTINCT_READBACK'):'IN_PROGRESS',revision_before:before.revision,revision_after:after.revision,cursor_before:before.cursor,cursor_after:after.cursor,completed_operations:completedOperations,created:counter.count,native_create_labels:counter.labels,max_creates:MAX_CREATES_PER_PHASE,projection_before:before.projection_sha256,projection_after:after.projection_sha256,protected_before:before.protected,protected_after:after.protected,validation,provenance:authorization.provenance,unknown_outcome:false,retry_allowed:false,next_operation:after.complete?'DISTINCT_LATER_READBACK':'AUTHORIZE_NEXT_EXACT_PROJECTION'};
}
