...
async function readActive(context,authorization){
  ...
  for(const key of ['session_id','task_id','writer_id','package_head','package_tree','triggered_by','lease_token'])invariant(String(marker[key])===String(authorization.provenance[key]),'ACTIVE_LEASE_IDENTITY_DRIFT',{key,expected:authorization.provenance[key],actual:marker[key]});
  invariant(marker.package_id===SPEC.package_id,'ACTIVE_LEASE_PACKAGE_DRIFT',{expected:SPEC.package_id,actual:marker.package_id});
  return marker;
}
function validateAuthorization(context,authorization,projection){
  ...
  invariant(authorization.provenance&&[...REQUIRED_PROVENANCE,'package_tree','lease_token'].every((key)=>typeof authorization.provenance[key]==='string'&&authorization.provenance[key]),'AUTH_PROVENANCE_INCOMPLETE');
...
function nativeCreate(counter,label,operation){const value=operation();counter.count+=1;counter.labels.push(label);return value;}
async function nativeCreateAsync(counter,label,operation){const value=await operation();counter.count+=1;counter.labels.push(label);return value;}
function instanceOf(component,counter){const shape=nativeCreate(counter,'component.instance',()=>component.instance());invariant(shape,'LINKED_INSTANCE_CREATE_FAILED');return shape;}
...
async function createOperation(context,operation,index,counter){
  ...
  if(operation.kind==='shape'){
    const shape=nativeCreate(counter,'penpot.createEllipse',()=>penpot.createEllipse());setGeometry(shape,{x:main.x,y:main.y,width:96,height:96,name:`${asset.short_name} · background`});shape.fills=[{fillColor:asset.presentation.background}];shape.strokes=[{strokeColor:asset.presentation.ring,strokeWidth:2}];mark(shape,operation.id,operation.role,{'source-bound':'true'});append(main,shape);return;
  }
  if(operation.kind==='artwork'){
    const handle=await exactAsset(context,asset);let shape;
    let image=null;
    if(asset.source.media_type==='image/svg+xml'){shape=nativeCreate(counter,'penpot.createShapeFromSvg',()=>penpot.createShapeFromSvg(handle.svg));invariant(shape,'SVG_IMPORT_RETURNED_NULL',asset.source.path);}
    else{image=await nativeCreateAsync(counter,'penpot.uploadMediaData',()=>penpot.uploadMediaData(`${asset.semantic_id}-${asset.source.sha256.slice(0,12)}`,handle.data,asset.source.media_type));invariant(image&&typeof image.data==='function','IMAGE_UPLOAD_RETURNED_INVALID_DATA',asset.source.path);const nativeBytes=await image.data();invariant(nativeBytes instanceof Uint8Array&&nativeBytes.byteLength===asset.source.bytes,'NATIVE_IMAGE_DATA_BYTES_DRIFT',asset.source.path);invariant(sha256Portable(nativeBytes)===asset.source.sha256,'NATIVE_IMAGE_DATA_SHA256_DRIFT',asset.source.path);shape=nativeCreate(counter,'penpot.createRectangle',()=>penpot.createRectangle());shape.fills=[{fillOpacity:1,fillImage:image}];}
  ...
  const shape=instanceOf(component,counter);configureLinked(shape,component,operation,416+col*(cellWidth+24)+(cellWidth-operation.tier)/2,256+row*(cellHeight+24)+(cellHeight-operation.tier)/2,operation.tier,operation.tier,`${asset.semantic_id}/tier-${operation.tier}`);mark(shape,operation.id,operation.role,{'semantic-slot':'linked_review_instances','semantic-id':asset.semantic_id,'tier-px':String(operation.tier),'linked-order':String(order+1),'source-sha256':asset.source.sha256});append(parentShape(page,`${LOGICAL_PAGE_ID}.dense-grid`),shape);
}
...
export async function executeMedallionsInstitutionsAR2NativePhase(context,authorization){
  ...
  try{
    for(let index=before.cursor;index<PLAN.length;index+=1){const cost=phaseCost(PLAN[index]);if(counter.count+cost>MAX_CREATES_PER_PHASE)break;await readActive(context,authorization);const countBefore=counter.count;await createOperation(context,PLAN[index],index,counter);invariant(counter.count-countBefore===cost,'NATIVE_CREATE_COST_DRIFT',{index,id:PLAN[index].id,expected:cost,actual:counter.count-countBefore});completedOperations+=1;lastIndex=index;const page=findPage(context.penpot);invariant(operationExists(context.penpot,page,PLAN[index]),'CREATE_NOT_READ_BACK',{index,id:PLAN[index].id});}
