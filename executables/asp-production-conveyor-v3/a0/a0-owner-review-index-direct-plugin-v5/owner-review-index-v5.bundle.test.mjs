...
function loadBundle() {
  ...
  return sandbox.D0A0OwnerReviewIndexV5;
}
...
test('exact source and Atlas R2 binding are preserved', async () => {
  ...
  assert.equal(packageRecord.harness.head, 'bb26c3d966511b590e26c3992da458b68c16fe2a');
  assert.equal(packageRecord.harness.tree, '9546076fd074604d117b65afea0489b1b60ccd96');
...
test('V4 partial is repaired in place with awaited geometry before any next row creates', async () => {
  const { bundle, host, penpot, audit } = await hostFixture();
  const namespace = 'kenigevents-d0-a0-owner-review-index-v4';
  const page = penpot.__seedPage('dbfda7a0-9d19-80cd-8008-93e9632cea31');
  page.name = bundle.source.page.page_name;
  page.setSharedPluginData(namespace, 'page-key', bundle.source.page.page_key);
  page.setSharedPluginData(namespace, 'package-id', bundle.source.package_id);
  const asyncResize = (node) => {
    node.resize = async (width, height) => { await Promise.resolve(); node.width = width; node.height = height; };
    return node;
  };
  const root = asyncResize(makeNode('dbfda7a0-9d19-80cd-8008-93e99c725185', 'board'));
  root.name = bundle.source.page.root_name; root.x = 0; root.y = 0; root.width = 100; root.height = 100;
  root.setSharedPluginData(namespace, 'stable-id', bundle.source.page.root_key);
  root.setSharedPluginData(namespace, 'package-id', bundle.source.package_id);
  page.root.appendChild(root);
  const headerComponent = penpot.library.local.components.find((component) => component.id === bundle.source.page.header_component_id);
  const header = asyncResize(makeNode('dbfda7a0-9d19-80cd-8008-93e99c8723b3', 'board'));
  header.x = 64; header.y = 64; header.width = 100; header.height = 100; header.hidden = true;
  header.isComponentCopyInstance = () => true; header.component = () => headerComponent;
  header.setSharedPluginData(namespace, 'stable-id', 'atlas.header.instance');
  header.setSharedPluginData(namespace, 'package-id', bundle.source.package_id);
  root.appendChild(header);
  const title = asyncResize(makeNode('dbfda7a0-9d19-80cd-8008-93e99cc0cec1', 'text'));
  title.characters = '42 CURRENT ATLAS R2 PHYSICAL PAGES'; title.x = 64; title.y = 272; title.width = 1; title.height = 1;
  title.setSharedPluginData(namespace, 'stable-id', 'authority/title');
  title.setSharedPluginData(namespace, 'package-id', bundle.source.package_id);
