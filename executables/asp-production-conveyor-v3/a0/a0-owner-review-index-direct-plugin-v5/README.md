# A0 Owner Review Index direct plugin bundle v5
...
use global `D0A0OwnerReviewIndexV5`:
...
V5 uses a portable in-bundle UTF-8/SHA-256 implementation (no `TextEncoder` or
...
V5 invokes the native `component.mainInstance()` reader. It embeds only the
...
## V5 in-place partial recovery

V5 supersedes V4 after native receipt 5511471375. It discovers the exact V4 page/root/header/title stable IDs, awaits native async `resize()`, clears the header hidden flag, verifies geometry after each repair, and does not create subsequent rows until the existing scaffold has settled. It never deletes or recreates the partial IDs.
